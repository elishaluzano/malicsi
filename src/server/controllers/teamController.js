...
exports.getTeamStats = (req,res) => {
	connection.query('select distinct event_title, team.name, team.team_id, event.event_id, (select count(record) from teamPlaysGame where record = "WIN" and team_id_play = ?) as wins, (select count(record) from teamPlaysGame where record = "LOSE" and team_id_play = ?) as losses from event join team join teamPlaysGame on event_id = event_id_key and team_id = ?',[ req.params.id,req.params.id,req.params.id,req.params.id,req.params.id ], function(err, rows, fields){
		if (err) {
            console.log(err);
            res.send(err);
         }
